...
class HajjController extends Controller
{
    ...
    {
        $sublist = HajjSublist::where('checklist_id',$id)->get();
    ...
    public function storeHajjProfileInformation(Request $request): JsonResponse
    {
        ...
        if ($validator->fails()) {
            ...
            if ($storeProfile) {
                ...
                event(new HajjProfileCreateEvent(HajjProfile::query()->latest()->first()));
    ...
    public function updateHajjProfileInformation(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id' => ['required', 'integer'],
            'customer_id' => ['required', 'integer'],
        ...
        if ($validator->fails()) {
            ...
            if (HajjProfile::where('id', $request->id)->exists()) {
                $profile = HajjProfile::findOrFail($request->id);
                ...
                if ($updateProfile) {
                    $profileInfo = HajjProfile::where('id', $request->id)->get();
    ...
    public function deleteHajjProfile($id): JsonResponse
    {
        if (HajjProfile::where('id', $id)->exists()) {
            $profile = HajjProfile::findOrFail($id);
            if ($profile) {
                $deleteProfile = $profile->delete();
                if ($deleteProfile) {
                    return response()->json(['success' => true, 'message' => 'Hajj profile is deleted successfully',], 200);
                } else {
