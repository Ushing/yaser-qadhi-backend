...
{
    protected QuranRecitationQuery $query;
    protected string $redirectUrl;
    const moduleDirectory = 'admin.quran-recitations.';

    public function __construct(QuranRecitationQuery $quranRecitationQuery)
    {
        $this->middleware('auth');
        $this->redirectUrl = 'admin/quran_recitations';
    ...
    {
        $data = [
            'moduleName' => 'Quran Fast Recitation',
    ...
    {
        $data = [
            'moduleName' => 'Quran Fast Recitation Create',
        ];
    ...
    public function store(SurahRecitationRequest $request): RedirectResponse
    {
        ...
        if ($quranRecitation) {
            alert()->success('Quran Fast Recitation', 'Item Created Successfully');
            return redirect()->route('admin.quran_recitations.index');
        } else {
            alert()->error('Quran Fast Recitation', 'Failed To Create');
            return redirect()->route('admin.quran_recitations.index');
        }
    }
    ...
    public function show(int $id): View
    {
        ...
        $data = [
            'moduleName' => 'Quran Fast Recitation Details',
    ...
    public function edit(int $id): View
    {
        $quranRecitation = $this->query->find($id);
        $data = [
            'moduleName' => 'Quran Fast Recitation Edit',
            'quranRecitation' => $quranRecitation,
    ...
    {
        $content = QuranRecitation::find($id);
        $quranRecitation = $this->query->updateQuranRecitation($request, $content);
        if ($quranRecitation) {
            alert()->success('Quran Fast Recitation', 'Item Updated Successfully');
            return redirect()->route('admin.quran_recitations.index');
        } else {
            alert()->error('Quran Fast Recitation', 'Failed To Update');
            return redirect()->route('admin.quran_recitations.index');
        }
    }
    ...
    public function destroy($id): JsonResponse
    {
        ...
        if ($quranRecitation->video) {
            unlink('uploads/quranRecitations/videos/' . $quranRecitation->video);
    ...
    public function statusChange($id): RedirectResponse
    {
        ...
        if ($quranRecitation) {
            if ($quranRecitation->status == 1) {
                alert()->success('Quran Fast Recitation Module', 'Item Status Is Active');
            }
            if ($quranRecitation->status == 0) {
                alert()->success('Quran Fast Recitation Module', 'Item Status Is Inactive');
            }
        } else {
            alert()->error('Quran Fast Recitation Module', 'Failed To Update Status');
        }
        return redirect()->route('admin.quran_recitations.index');
    }
    ...
    public function uploadRecitationVideo(Request $request): array
    {
        ...
        if ($fileReceived->isFinished()) { // file uploading is complete / all chunks are uploaded
            ...
            if (!file_exists('uploads/quranRecitations/videos')) {
                mkdir('uploads/quranRecitations/videos', 0777, true);
            }
            $file->move('uploads/quranRecitations/videos', $fileName);
            return [
                'filename' => $fileName
            ];
        }
    ...
    public function uploadRecitationAudio(Request $request): array
    {
        ...
        if ($fileReceived->isFinished()) { // file uploading is complete / all chunks are uploaded
            ...
            $fileName = str_replace('.' . $extension, '', $file->getClientOriginalName()); //file name without extenstion
            $fileName .= '_' . md5(time()) . '.' . $extension; // a unique file name
            if (!file_exists('uploads/quranRecitations/audio')) {
                mkdir('uploads/quranRecitations/audio', 0777, true);
            }
            $file->move('uploads/quranRecitations/audio', $fileName);
            return [
                'filename' => $fileName
            ];
        }
