...
class YasirLectureCategoryListQuery extends BaseQuery
{
    /**
     * @var $model
     */
    ...
    public function __construct(YasirLectureCategory $YasirLectureCategory)
    {
        $this->model = $YasirLectureCategory;
    }
    ...
    public function getAllData($request): JsonResponse
    {
        ...
        return DataTables::of($query)
            ->addColumn('action', function ($row) {
                $actions = '';
                $actions .= '<a href="' . route('admin.yasir-lecture-category-list.edit', [$row->id]) . '" class="btn btn-success btn-sm ml-2 mt-2  " title="Edit"><i class="fa fa-edit"></i></a>';
            ...
            ->addColumn('status_change', function ($row) {
                ...
                $toggleAction .= '<a  title="' . $title . '" href="' . route('admin.yasir-lecture-category-list.status', [$row->id]) . '" class="btn btn-md btn-sm ml-2 font-weight-bold  ' . $color . ' btn-icon ml-2" >' . $status . '</a>';
                return $toggleAction;
            })
    ...
    public function saveRecitation($request): string
    {
        ...
        try {
            ...
            return $this->model->create([
                'title' =>$request->title,
                'yasir_lecture_id' =>$request->yasir_lecture_id,
