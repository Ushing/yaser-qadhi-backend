...
class QuranSlowRecitationQuery extends BaseQuery
{
    /**
     * @var $model
     */
    ...
    public function __construct(QuranSlowRecitation $quranRecitation)
    {
    ...
    public function getAllData($request): JsonResponse
    {
        ...
        return DataTables::of($query)
            ->addColumn('action', function ($row) {
                $actions = '';
                $actions .= '<a href="' . route('admin.quran_slow_recitations.edit', [$row->id]) . '" class="btn btn-success btn-sm ml-2 mt-2  " title="Edit"><i class="fa fa-edit"></i></a>';
                $actions .= '<a href="' . route('admin.quran_slow_recitations.show', [$row->id]) . '" class="btn btn-info btn-sm ml-2 mt-2 " title="View"><i class="fas fa-eye-slash"></i></a>';
            ...
            ->addColumn('status_change', function ($row) {
                ...
                $toggleAction .= '<a  title="' . $title . '" href="' . route('admin.quran_slow_recitations.status', [$row->id]) . '" class="btn btn-md btn-sm ml-2 font-weight-bold  ' . $color . ' btn-icon ml-2" >' . $status . '</a>';
                return $toggleAction;
            })
