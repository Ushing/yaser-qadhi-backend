...
class MessageofQuranQuery extends BaseQuery
{
    /**
     * @var $model
     */
    ...
    public function __construct(MessageofQuran $MessageofQuran)
    {
        $this->model = $MessageofQuran;
    }
    ...
    public function getAllData($request): JsonResponse
    {
        ...
        return DataTables::of($query)
            ->addColumn('action', function ($row) {
                $actions = '';
                $actions .= '<a href="' . route('admin.message_of_qurans.edit', [$row->id]) . '" class="btn btn-success btn-sm ml-2 mt-2  " title="Edit"><i class="fa fa-edit"></i></a>';
                $actions .= '<a href="' . route('admin.message_of_qurans.show', [$row->id]) . '" class="btn btn-info btn-sm ml-2 mt-2 " title="View"><i class="fas fa-eye-slash"></i></a>';
            ...
            ->addColumn('status_change', function ($row) {
                ...
                $toggleAction .= '<a  title="' . $title . '" href="' . route('admin.message_of_qurans.status', [$row->id]) . '" class="btn btn-md btn-sm ml-2 font-weight-bold  ' . $color . ' btn-icon ml-2" >' . $status . '</a>';
                return $toggleAction;
            })
