...
class QuranInterviewQuery extends BaseQuery
{
    /**
     * @var $model
     */
    ...
    public function __construct(QuranInterview $quranInterview)
    {
        $this->model = $quranInterview;
    }
    ...
    public function getAllData($request): JsonResponse
    {
        ...
        return DataTables::of($query)
            ->addColumn('action', function ($row) {
                $actions = '';
                $actions .= '<a href="' . route('admin.quran_interviews.edit', [$row->id]) . '" class="btn btn-success btn-sm ml-2 mt-2  " title="Edit"><i class="fa fa-edit"></i></a>';
                $actions .= '<a href="' . route('admin.quran_interviews.show', [$row->id]) . '" class="btn btn-info btn-sm ml-2 mt-2 " title="View"><i class="fas fa-eye-slash"></i></a>';
            ...
            ->addColumn('status_change', function ($row) {
                ...
                $toggleAction .= '<a  title="' . $title . '" href="' . route('admin.quran_interviews.status', [$row->id]) . '" class="btn btn-md btn-sm ml-2 font-weight-bold  ' . $color . ' btn-icon ml-2" >' . $status . '</a>';
                return $toggleAction;
            })
    ...
    public function saveQuranInterview($request): string
    {
        try {
    ...
    public function updateQuranInterview($request, $quranInterview): string
    {
        try {
            if ($request->quran_audio != null) {
                $audioFileNameToStore = $request->quran_audio;
            } else {
                $audioFileNameToStore = $quranInterview->audio;
            }
            if ($request->quran_video != null) {
                $videoFileNameToStore = $request->quran_video;
            } else {
                $videoFileNameToStore = $quranInterview->video;
            }
            return $quranInterview->update([
                'title' => $request->title,
                'reference_id' => $request->reference_id ?? null,
                'status' => $request->status ?? 1,
                'audio' => $audioFileNameToStore,
