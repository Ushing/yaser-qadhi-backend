    ...
    <div class="content">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active"><a href="{{route('admin.yasir-lecture-category-list.edit',$arabicGrammarCategoryList->id)}}">Edit</a></li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-info">
                            <div class="card-header">
                                <h3 class="card-title">Lecture Category List Edit</h3>
                            </div>
                            <form action="{{ route('admin.yasir-lecture-category-list.update',$arabicGrammarCategoryList->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-2 col-md-6">
                                            <div class="form-group">
                                                <label for="exampleSelect1">Yasir Lecture<span class="text-danger">*</span></label>
                                                <select
                                                    class="form-control selectpicker {{ $errors->has('yasir_lecture_id') ? 'is-invalid' : '' }}"
                                                    name="yasir_lecture_id" id="yasir_lecture_id" data-size="5" data-live-search="true">

                                                    <option value="">Select Yasir Lecture</option>
                                                    @foreach ($arabicGrammar as $Arabic_grammar)
                                                        <option value="{{ $Arabic_grammar->id }}"
                                                            {{ $Arabic_grammar->yasir_lecture_id == $Arabic_grammar->id ? 'selected' : '' }}>
                                                            {{ $Arabic_grammar->title }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                @if ($errors->has('yasir_lecture_id'))
                                                    <div class="invalid-feedback">{{ $errors->first('yasir_lecture_id') }}</div>
                                                @endif
                                            </div>
                                        </div>
        ...
        <script type="text/javascript">
            ...
            let resumableVideoFile = new Resumable({
                target: '{{ route('admin.yasir-lecture-category-list.upload.video') }}',
            ...
            let resumableAudioFile = new Resumable({
                target: '{{ route('admin.yasir-lecture-category-list.upload.audio') }}',
                query: {_token: '{{ csrf_token() }}'},
